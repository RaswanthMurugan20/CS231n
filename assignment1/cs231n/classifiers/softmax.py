...
def softmax_loss_vectorized(W, X, y, reg):
    ...
    y_new[range(X.shape[0]),y] = 1
    
    block = scores/div_constant
    
    scores_new = np.zeros(scores.shape)
    scores_new[range(X.shape[0]),y] = scores[range(X.shape[0]),y]
    loss = -np.sum(log(scores_new/div_constant))/(X.shape[0]) + reg*np.sum(np.square(W))
    
    
    dW = np.dot(X.T,(block-y_new))/(X.shape[0])
    dW = dW + 2*reg*W
